...
def custom_collate_fn(batch):
    ...
    labels, masks, bboxes, orig_sizes = zip(*targets)

    images = torch.stack(images)
    labels = torch.tensor(labels)
    masks = torch.stack(masks)
    return images, (labels, masks, list(bboxes), list(orig_sizes))


class OxfordPetsWithBBoxes(OxfordIIITPet):
    def __init__(self, root, split="trainval", transform=None, target_transform=None, download=False):
        super().__init__(root=root, split=split, target_types=["binary-category", "segmentation"],
                         transform=transform, target_transform=target_transform, download=download)

    def __getitem__(self, index):
        # Get transformed image and targets
        image, (label, seg_mask) = super().__getitem__(index)

        # Load original image to get original size
        pil_image = Image.open(self._images[index]).convert("RGB")
        original_size = pil_image.size  # (width, height)

        # Load bounding box
        bbox = self._load_bbox(self._images[index])

        if bbox is None:
            return self.__getitem__((index + 1) % len(self))

        return image, (label, seg_mask, bbox, original_size)

    def _load_bbox(self, image_path):
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        xml_path = os.path.join(self.root, "annotations", "xmls", base_name + ".xml")

        if not os.path.exists(xml_path):
            #print(f"[WARNING] Missing annotation: {xml_path} — skipping this sample.")
            return None
...
class SingleStageWeaklySupervisedModel(nn.Module):
    def __init__(self, backbone: nn.Module, classification_head: nn.Module, segmentation_head: nn.Module, 
                 weight_seg: float = 1.0, weight_cls: float = 1.0, target_threshold = 0.3, mask_confidence = 0.5):
        """
        Args:
    ...
    def forward(self, images, labels=None, masks=None, bboxes=None, original_sizes=None):
        """
        Args:
        ...
        #---SIMPLER TWO TOWER METHOD:---
            # Extract multi-scale features from EfficientNetB0 backbone
        ...
        # Apply smoothing
        cam0_up = apply_gaussian_smoothing(cam0_up, kernel_size=SMOOTH_KERNEL, sigma=SMOOTH_SIGMA)
        cam1_up = apply_gaussian_smoothing(cam1_up, kernel_size=SMOOTH_KERNEL, sigma=SMOOTH_SIGMA)
        cam2_up = apply_gaussian_smoothing(cam2_up, kernel_size=SMOOTH_KERNEL, sigma=SMOOTH_SIGMA)
        cam3_up = apply_gaussian_smoothing(cam3_up, kernel_size=SMOOTH_KERNEL, sigma=SMOOTH_SIGMA)
        ...
        if bboxes is not None and original_sizes is not None:
            B, C, H, W = cam_combined.shape
            for i in range(B):
                ...
                if isinstance(bbox, torch.Tensor):
                    ...
                    raise TypeError(f"Unexpected bbox format: {type(bbox)}")

                if len(bbox) != 4:
                    raise ValueError(f"Expected 4 values in bbox, got {len(bbox)}: {bbox}")
                
                orig_w, orig_h = original_sizes[i]  # (W, H)

                xmin, ymin, xmax, ymax = bbox

                # Scale bbox coordinates from original image (e.g., 224x224) to CAM shape
                x_scale = W / orig_w
                y_scale = H / orig_h
                ...
                # Create a mask: 1 inside bbox, 0 outside
                mask = torch.zeros_like(cam_combined[i])
                mask[:, y1:y2, x1:x2] = 1.0

                # Apply mask to CAM (hard masking - could NOT be the move)
                cam_combined[i] = cam_combined[i] * mask

                #Trying soft masking: 

                #cam_combined[i] = cam_combined[i] + self.mask_confidence * mask
                #cam_combined[i] = cam_combined[i] * mask
                #cam_combined[i] += 0.9 * mask  # You can tune this multiplier (e.g., 0.1 - 0.5)
                #cam_combined[i] = cam_combined[i].clamp(0, 1)  # Ensure values stay in [0, 1]
...
def train_single_stage_model(model, train_loader, val_loader = None, classification_loss_fn = None, 
                             ...
                             device="cpu", num_epochs=10, print_every=1, initial_threshold=0.5, thresh_exp = False, decay_rate = 0.95):
    """
    ...
    for epoch in range(num_epochs):
        ...
        if thresh_exp: 
              # how fast to decay (e.g. 0.95 = 5% drop per epoch)
            #dynamic_threshold = initial_threshold * (decay_rate ** epoch)
            # keep it constant to check if this is why terminal is dying
        ...
        for batch_idx, (images, (labels, masks, bboxes, original_sizes)) in enumerate(train_loader):
                    #for images, (labels, masks) in enumerate(train_loader):

            for i, bbox in enumerate(bboxes):
                if len(bbox) != 4:
                    print(f"[DEBUG] Batch {batch_idx}, item {i}, invalid bbox: {bbox}")
                    raise ValueError(f"Expected 4 values in bbox, got {len(bbox)}")
            ...
            outputs = model(images, labels=labels, masks=masks,
                            bboxes=bboxes, original_sizes=original_sizes)
            ...
            del images, labels, masks, logitsCls, logitsSeg, pseudoMasks, outputs
            torch.cuda.empty_cache()  # works even if you're on CPU, it's a safe no-op
        ...
        if (epoch + 1) % print_every == 0:
            ...
            print(f"→ Epoch {epoch+1}: Dynamic CAM threshold = {dynamic_threshold:.4f}")
...
def validate_single_stage_model( model, val_loader, classification_loss_fn, segmentation_loss_fn, 
    ...
    with torch.no_grad():
       for batch_idx, (images, (labels, masks, bboxes, original_sizes)) in enumerate(val_loader):
            ...
            logits_cls = outputs["logits_cls"].detach()
            logits_seg = outputs["logits_seg"].detach()
            ...
            all_pred_labels.append(preds.detach().cpu().numpy())

            # Get segmentation predictions
            preds_seg = torch.argmax(logits_seg, dim=1)  # (B, H, W)
            all_seg_true.append(masks.detach().cpu().numpy())
            all_seg_pred.append(preds_seg.detach().cpu().numpy())

            del images, labels, masks, logits_cls, logits_seg, preds, preds_seg, outputs    
            torch.cuda.empty_cache()  # works even if you're on CPU, it's a safe no-op
...
def visualize_cam_with_bbox(image, cam, bbox, original_size):
    fig, ax = plt.subplots()
    ax.imshow(image.permute(1, 2, 0))
    ax.imshow(cam, cmap='jet', alpha=0.5)

    # Original size and target display size (e.g., 224x224)
    orig_w, orig_h = original_size
    disp_h, disp_w = image.shape[1:]
...
def decode_segmentation_mask(mask):
    ...
    h, w = mask.shape
    rgb = np.zeros((h, w, 3), dtype=np.uint8)
    rgb[mask == 0] = [128, 0, 128]   # Background: Purple
    rgb[mask == 1] = [255, 255, 0]   # Cat: Yellow
    rgb[mask == 2] = [0, 255, 0]     # Dog: Green
    return rgb

def plot_all_visualizations(images, cams, pseudo_masks, preds_seg, masks=None, bboxes=None, original_sizes=None,  num_images=4):
    """
    Combines all visualization types into one subplot per image.
    """
    images = images.cpu()
    cams = cams.cpu()
    pseudo_masks = pseudo_masks.cpu()
    ...
    cam_max = torch.max(cam_upsampled, dim=1)[0]  # (B, 224, 224)

    pseudo_masks_up = F.interpolate(pseudo_masks.unsqueeze(1).float(), size=(224, 224), mode='nearest').squeeze(1)

    for i in range(min(num_images, images.shape[0])):
        fig, axs = plt.subplots(2, 4, figsize=(20, 10))

        # Row 1
        axs[0, 0].imshow(TF.to_pil_image(images[i]))
        axs[0, 0].set_title("Input Image")
        axs[0, 0].axis("off")

        axs[0, 1].imshow(TF.to_pil_image(images[i]))
        axs[0, 1].imshow(cam_max[i], cmap='jet', alpha=0.5)
        axs[0, 1].set_title("CAM (with BBox)" if bboxes[i] is not None else "CAM Overlay")
        axs[0, 1].axis("off")

        if bboxes[i] is not None and original_sizes is not None:
            orig_w, orig_h = original_sizes[i]  # (W, H)
            disp_h, disp_w = images[i].shape[1:]  # After transform (likely 224x224)
        ...
        axs[0, 2].imshow(cams[i].max(dim=0)[0], cmap='jet')
        axs[0, 2].set_title("Raw CAM (low-res)")
        axs[0, 2].axis("off")

        axs[0, 3].imshow(pseudo_masks[i], cmap='viridis')
        axs[0, 3].set_title("Raw Pseudo Mask")
        axs[0, 3].axis("off")

        # Row 2
        axs[1, 0].imshow(pseudo_masks_up[i], cmap='viridis')
        axs[1, 0].set_title("Upsampled Pseudo Mask")
        axs[1, 0].axis("off")

        pred_rgb = decode_segmentation_mask(preds_seg[i].numpy())
        axs[1, 1].imshow(pred_rgb)
        axs[1, 1].set_title("Predicted Segmentation")
        axs[1, 1].axis("off")

        if masks is not None:
            mask_rgb = decode_segmentation_mask(masks[i].numpy())
            axs[1, 2].imshow(mask_rgb)
            axs[1, 2].set_title("Ground Truth")
            axs[1, 2].axis("off")
        else:
            axs[1, 2].axis("off")

        axs[1, 3].axis("off")  # Optional slot for extra plots
