...
class SingleStageWeaklySupervisedModel(nn.Module):
    ...
    def forward(self, images, labels=None, masks=None):
        ...
        # Extract intermediate features from EfficientNet backbon
        ...
        # labels[confidence < self.threshold] = ignore_index  # shape: (B, H, W)
        # pseudo_masks = labels
        # pseudo_masks = F.interpolate(pseudo_masks.unsqueeze(1).float(), size=(224, 224), mode="nearest").squeeze(1).long() #something to do with upscaling
        ...
        # print(f"[DEBUG] Number of features returned: {len(feats)}")
        # for i, f in enumerate(feats):
        #     print(f"Feature {i} shape: {f.shape}")
        # feat1 = feats[-3]  # 28x28 resolution
        # feat2 = feats[-2]  # 14x14 resolution

        # # Classification using deeper feature

        # # Optional projection (adapt if needed)
        # W_proj1 = classifier_weights[:, :feat1.shape[1]]  # slice for feat1 if dims match

        # # Generate CAMs at both resolutions
        # cam1 = generate_cam(feat1, logits_cls, W_proj1, apply_smoothing = False)       # 28x28
        # cam2 = generate_cam(feat2, logits_cls, classifier_weights, apply_smoothing = False)  # 14x14

        # # Upsample both CAMs to match input size and combine
        ...
        # cam_combined = 0.3 * cam1_up + 0.7 * cam2_up

        # cam_combined = apply_gaussian_smoothing(cam_combined, kernel_size=7, sigma=2.0)
        # #cam_combined = apply_gaussian_smoothing(cam_combined, kernel_size=7, sigma=2.0)
        ...
        # Segmentation head takes feat2 (or fuse if you want more context)
        ...
        # # Generate CAM
        # cam = generate_cam(features, logits_cls, classifier_weights, apply_smoothing=False)

        # # Upsample to input size
        # cam_upsampled = F.interpolate(cam, size=(224, 224), mode="bilinear", align_corners=False)

        # # Create pseudo masks from CAM
        # confidence, labels = torch.max(cam_upsampled, dim=1)
        # ignore_index = 255
        # threshold = compute_dynamic_threshold(confidence, target_ignore_ratio=self.target_threshold)
        # labels[confidence < threshold] = ignore_index
        # pseudo_masks = labels
