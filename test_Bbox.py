from Single_stage_tools_Bbox import *
import torch
...
class EfficientNetB0Backbone(nn.Module):
    def __init__(self):
        ...
        self.out_channels = model.feature_info[-1]['num_chs']  # 14x14 resolution

    def forward(self, x):
        feats = self.features(x)
        return feats[-1]  # (B, C, 7, 7)
...
class SimpleClassificationHead(nn.Module):
    def __init__(self, in_channels=512, num_classes=2, dropout_p = 0.5):
        # Oxford-IIIT Pet has 37 classes.
        super(SimpleClassificationHead, self).__init__()
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
...
class SimpleSegmentationHead(nn.Module):
    def __init__(self, in_channels=512, num_seg_classes=3):
        # For binary segmentation (foreground vs. background)
        super(SimpleSegmentationHead, self).__init__()
...
if __name__ == "__main__":
    ...
    dataset = download_data(pth="./data/oxford-iiit-pet", split="trainval")
    ...
    cls_head = SimpleClassificationHead(in_channels=backbone.out_channels, num_classes=2, dropout_p = 0.3)

    seg_head = UNetSegmentationHead(in_channels=112, num_classes=3, dropout_p = 0.3)
    ...
    starting_thresholds = [0.9,0.6,0.3]
    #decay_rates = [0.9,0.6,0.3]
    decay_rates = [0.5]
    mask_confidences = [0.1,0.2,0.3]

    for t in starting_thresholds: 
        for m in mask_confidences: 
            for d in decay_rates: 


                print()
                print(f"--- Training with Threshold: {t}, Mask Confidence: {m}, Decay Rate: {d} ---")
                print()
                model = SingleStageWeaklySupervisedModel(backbone, cls_head, seg_head, target_threshold = t, mask_confidence = m)
                ...
                # Grab a batch
                #images, (labels, masks, bboxes) = next(iter(val_loader))
                images, (labels, masks, bboxes, original_sizes) = next(iter(val_loader))
                ...
                with torch.no_grad():
                    ...
                    outputs = model(images, labels=labels, masks=masks, bboxes=bboxes)
                    pseudo_masks = outputs["pseudo_masks"]
                    logits_seg = outputs["logits_seg"]
                    preds_seg = torch.argmax(logits_seg, dim=1)

            #results_df.to_csv("grid_search_results.csv", index=False)
                
                # for i in range(len(images)):
                #     visualize_cam_with_bbox(images[i].cpu(), torch.max(cams[i], dim=0)[0].cpu(), bboxes[i])
                ...
                # visualize_raw_cams_and_pseudo_masks(images, cams, pseudo_masks, num_images = 8)
                # visualize_upsampled_cams_and_pseudo_masks(images, cams, pseudo_masks, num_images = 8)
                # visualize_segmentation_predictions(images, preds_seg, masks=masks, num_images=8)